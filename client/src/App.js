...
import FunctionalExample from "./views/FunctionalExample";
...
export default class App extends Component {
  ...
  getCartItemTotal = () => {
    ...
    for (const item in JSON.parse(localStorage.getItem("cart"))) {
      total += JSON.parse(localStorage.getItem("cart"))[item].quantity;
    }
    return total;
  };
  ...
  getCartTotalPrice = () => {
    ...
    for (const item in JSON.parse(localStorage.getItem("cart"))) {
      total +=
        JSON.parse(localStorage.getItem("cart"))[item].price *
        JSON.parse(localStorage.getItem("cart"))[item].quantity;
    }
    return total;
  };
  ...
  render() {
    return (
      <div>
        ...
        <Switch>
          <ProtectedRoute token={this.state.token} exact path="/checkoutSuccess" render={()=><CheckoutSuccess clearCart={this.clearCart}/>}/>
          <ProtectedRoute
            token={this.state.token}
            exact
            path="/"
          ...
          <ProtectedRoute
            ...
            exact
            path="/page3"
            render={() => <Page3 user={this.state.user} />}
          />
          <ProtectedRoute
            token={this.state.token}
            exact
          ...
          <ProtectedRoute
            token={this.state.token}
            exact
            path="/functionalexample"
            render={() => <FunctionalExample />}
          />
          <ProtectedRoute
            token={this.state.token}
            exact
          ...
          <ProtectedRoute
            ...
            path="/item:id"
            render={(props) => (
              <SingleItem addToCart={this.addToCart} {...props} />
            )}
          />
          <ProtectedRoute
            token={this.state.token}
